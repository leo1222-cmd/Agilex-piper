...
def move_joint_slowly(
    ...
    for i in range(max_iters):
        ...
        if i % 10 == 0:
            print(f"[MOVE {i:04d}] err={err:.3f} deg | {joint_readable(current)}")

        if err <= tol_deg:
            print(f"[REACHED] err={err:.3f} deg")
            return True
    ...
    print("[WARN] move_joint_slowly reached max_iters but not within tolerance.")
    current = read_joint_raw(piper)
...
def main():
    ...
    print("2. 这个脚本会让从臂直接运行到示教夹取关节角。")
    print("3. 第一次不要加 --auto。")
    print("4. 手放急停附近。")
    print("============================================")

    if not args.auto:
        input("确认从臂路径安全、夹爪和相机线不会碰撞，按 Enter 开始运行到示教关节角...")
    ...
    if not ok_move:
        print("[STOP] 关节角未到位，不进行 ArUco 校验。")
        return

    if args.skip_verify:
        print("[DONE] 已跳过 ArUco 校验。")
        return

    if not args.auto:
        input("从臂已到示教关节角，确认相机画面安全，按 Enter 开始 ArUco 到位校验...")
    ...
    if ok_verify:
        print("[PASS] 关节复现 + ArUco 到位校验通过。下一步可以加入夹爪闭合。")
    else:
        print("[FAIL] 关节复现后 ArUco 相对位姿误差过大，先不要夹爪。")
